...
class Gui(): # class to deal with the visual elements for the human player
    ...
    def draw(self) -> None:
        ...
        def color(x):
            return Gui.CB if x % 2 == 0 else Gui.CD 
        for y, row in enumerate(self.board):
            for x, spot in enumerate(row):
                i = x + y
                c = color(i)
                ...
                if rect[0] < mouse_x < rect[2] and rect[1] < mouse_y < rect[3]:
                    c = (min(255, c[0] + 15), min(255, c[1] + 15), min(255, c[2] + 15))
                # Verificar se a cor é válida
                if not isinstance(c, tuple) or len(c) != 3 or not all(isinstance(x, int) and 0 <= x <= 255 for x in c):
                    c = (255, 255, 255)  # Cor padrão branca
